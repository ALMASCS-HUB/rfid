...
// Initialize Firebase
const serviceAccount = require('/home/akhil/Desktop/oiu/firebase.json');
...
// API endpoint for attendance data (protected)
  ...
  try {
    ...
    for (const [date, dateInfo] of Object.entries(attendanceData)) {
      for (const [uid, uidInfo] of Object.entries(dateInfo)) {
        if (uidInfo.records) {
          for (const [recordId, recordInfo] of Object.entries(uidInfo.records)) {
            if (recordInfo.login_time) { // Only process records with a login_time
              ...
              if (userInfo) {
                ...
                const status = exit_time ? 'logged out' : 'present';
